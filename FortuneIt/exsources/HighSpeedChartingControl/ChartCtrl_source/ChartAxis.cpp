...
CChartAxis::CChartAxis() 
 : m_pParentCtrl(NULL), m_bIsHorizontal(true), m_bIsInverted(false), 
   ...
   m_dTickLabelOrientationInDegree(0.0), m_bEnableGrid(true)
{
...
void CChartAxis::Draw(CDC *pDC)
{
	...
	NewFont.CreateFont(-MulDiv(m_nFontSize, pDC->GetDeviceCaps(LOGPIXELSY), 72)/10,0, m_dTickLabelOrientationInDegree*10, m_dTickLabelOrientationInDegree*10, FW_DONTCARE,false,false,false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, m_strFontName.c_str());
						  ...
						  double dTickVal)
{
	...
	if (m_bIsHorizontal)
	{
		...
		if (!m_bIsSecondary)
		{
			...
			if (bLabelOnAxis)
			{
				if (fabs(GetTickLabelOrientationInDegree())<1)
					pDC->ExtTextOut(lLabelPos - TextSize.cx / 2, m_AxisRect.top + 5,
						ETO_CLIPPED | ETO_OPAQUE, NULL, strBuffer.c_str(), NULL);
				else
				{
					long x = lLabelPos - TextSize.cy / 2;
					long y = m_AxisRect.top + 5;
					double Deg2Rad = 3.14159265359/180;
					double deltaX = TextSize.cx * cos(m_dTickLabelOrientationInDegree * Deg2Rad);
					double deltaY = TextSize.cx * sin(m_dTickLabelOrientationInDegree * Deg2Rad);
					x -= deltaX;
					y += deltaY;
