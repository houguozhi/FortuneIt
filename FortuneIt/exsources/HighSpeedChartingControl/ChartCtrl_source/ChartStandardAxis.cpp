...
CChartStandardAxis::CChartStandardAxis()
 : CChartAxis(), m_dFirstTickValue(0), 
   m_dTickIncrement(1.0), m_uDecCount(0),
	m_strDTTickFormat(_T("%Y%m%d(%w) %H:%M")),
	m_dMinTickIncrement(0.0)
{
}
...
TChartString CChartStandardAxis::GetTickLabel(double TickValue) const
{
	int index = TickValue;
	double time = 0;
	if (index >= 0 && index < m_pRelatedSeries.front()->GetPointsCount())
		time = ((CChartCandlestickSerie *)m_pRelatedSeries.front())->GetPoint(index).GetTime();

	COleDateTime tickTime((DATE)time);
...
void CChartStandardAxis::RefreshTickIncrement()
{
	...
	if (MinTickIncrement < m_dMinTickIncrement)
		MinTickIncrement = m_dMinTickIncrement;
