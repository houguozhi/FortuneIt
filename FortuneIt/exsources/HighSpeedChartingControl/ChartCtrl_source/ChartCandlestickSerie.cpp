...
CChartCandlestickSerie::CChartCandlestickSerie(CChartCtrl* pParent) 
  : CChartSerieBase<SChartCandlestickPoint>(pParent), m_iCandlestickWidth(7), m_Cycle(6)
{
	m_bShadow = true;
}
...
void CChartCandlestickSerie::AddPoint(double Time, 
									  double Open,
									  double High, 
									  double Low, 
									  double Close)
{
	SChartCandlestickPoint NewPoint(GetPointsCount(), Time, Open, High, Low, Close);
	CChartSerieBase<SChartCandlestickPoint>::AddPoint(NewPoint);
}

void CChartCandlestickSerie::SetWidth(int Width, bool bRefreshCtrl /*= true*/)
{
											...
											unsigned& uIndex) const
{
	...
	for (unsigned i=uFirst ; i < uLast ; i++)
	{
		...
		int ScreenXVal = m_pHorizontalAxis->ValueToScreen(Point.GetX());
...
void CChartCandlestickSerie::DrawLegend(CDC* pDC, const CRect& rectBitmap) const
{
	...
	PositivePen.CreatePen(PS_SOLID,1,m_SerieColor);
	BrushFill.CreateSolidBrush(m_SerieColor);
	CBrush* pOldBrush = pDC->SelectObject(&BrushFill);
	CPen* pOldPen = pDC->SelectObject(&PositivePen);
...
void CChartCandlestickSerie::Draw(CDC* pDC)
{
	...
	if (pDC->GetSafeHdc())
	{
		AdjustCankestickWidthAccordingToScreen();

		ShadowBrush.CreateSolidBrush(m_ShadowColor);
		PositivePen.CreatePen(PS_SOLID, 1, RGB(255, 0, 0));
		NegtivePen.CreatePen(PS_SOLID, 1, m_SerieColor);
		...
		for (m_uLastDrawnPoint;m_uLastDrawnPoint<GetPointsCount();m_uLastDrawnPoint++)
		{
			...
			DrawCandleStick(pDC, m_uLastDrawnPoint);
		}
...
void CChartCandlestickSerie::DrawCandleStick(CDC *pDC, int pointIndex)
{
	SChartCandlestickPoint Point = GetPoint(pointIndex);
	...
	{
		pDC->SelectObject(&NegtivePen);
		pDC->SelectObject(&BrushFill);
	}
	else
	{
		pDC->SelectObject(&PositivePen);
...
int CChartCandlestickSerie::AdjustCankestickWidthAccordingToScreen()
{
	int oldWidth = GetWidth();
	int newWidth = oldWidth;
	int cnt = GetVisibalePointsCount();
	if (cnt)
	{
		newWidth = (m_pHorizontalAxis->GetAxisLenght() / cnt) * 1 / 2;
